...
fun main() {
    ...
    val dizhiquence = listOf("0","子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥")
...
fun getLunarData(year: Int, month: Int, day: Int, hour: Int, minute: Int, second: Int, gender: String): Triple<EightChar, LunarHour, List<Int>> {
    ...
    val bagualist = listOf(tianchengshu,renchengshu,dichengshu)
        return Triple(eightChar, lunarHour,bagualist)
}


    val (eightChar, lunarHour ,bagualist) = getLunarData(1939, 8, 4, 16, 37, 0, "男")
    println(eightChar)
    println(lunarHour)
    println(bagualist)
